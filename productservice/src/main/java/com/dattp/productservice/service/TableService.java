...
public class TableService {
    ...
    public List<PeriodsTimeBookedTableDTO> getFreeTimeOfTable(Date fromI, Date toI, Pageable pageable,
            String accessToken) {
        List<PeriodsTimeBookedTableDTO> list = new ArrayList<>();
        // lấy danh sách các bàn, trong đó có thời gian đặt của từng bàn
        // String url =
        // ApplicationConfig.HOST_ORDER_SERVICE+"/api/booking/booked_table/get_all_period_rent_table?from={from}&to={to}";
        // HttpHeaders headers = new HttpHeaders();
        // headers.set("access_token", accessToken);
        // HttpEntity<String> request = new HttpEntity<>(headers);
        // ResponseEntity<ResponseListTableFreeTimeDTO> response =
        ...
        // final String url1 =
        // ApplicationConfig.HOST_ORDER_SERVICE+"/api/booking/booked_table/get_period_rent_table/{id}?from={from}&to={to}";
        // tableRepository.findAllNotIn(listNotIn,pageable).getContent().stream().forEach((t)->{
        // PeriodsTimeBookedTableDTO periodsTimeBookedTableDTO = new
        // PeriodsTimeBookedTableDTO();
        ...
        // id,from,to
        // );
        // LinkedHashMap<String,List<Object>> map = (LinkedHashMap<String,
        // List<Object>>) resp1.getBody().getData();
        // map.get("times").stream().forEach((obj)->{
        // LinkedHashMap<String,String> time = (LinkedHashMap<String, String>) obj;
        // try {
        // PeriodTimeResponseDTO timeResp = new
        // PeriodTimeResponseDTO(format.parse(time.get("from")),
        // format.parse(time.get("to")));
        ...
        // });
        Page<TableE> page = tableRepository.findAll(pageable);
        while(true){
            page.toList().forEach((t) -> {
                // table
                PeriodsTimeBookedTableDTO periodsTimeBookedTableDTO = new PeriodsTimeBookedTableDTO();
                BeanUtils.copyProperties(t, periodsTimeBookedTableDTO);
                //get period booked of table
                String keyPeriodbookedTable = String.format("table:%d:period_booked", t.getId());
                periodsTimeBookedTableDTO.setTimes(new ArrayList<>());
                List<Object> times = redisService.getData(keyPeriodbookedTable, 0, -1);
                // check
                if(times!=null){
                    boolean ok = true;
                    for(Object obj : times){
                        PeriodTimeResponseDTO time = (PeriodTimeResponseDTO)obj;
                        if(!(time.getTo().compareTo(fromI)<=0||toI.compareTo(time.getFrom())<=0)){
                            ok = false;
                            break;
                        }
                    }
                    if(ok) list.add(periodsTimeBookedTableDTO);
                }else list.add(periodsTimeBookedTableDTO);// period booked of table is empty
            });
            if(!page.hasNext()) break;
            if(list.size()>page.getSize()) break;
    ...
    public boolean addComment(Long tableId, CommentTable comment) {
        ...
        try {
            if (commentTableRepository.findByTableIdAndUserId(tableId, comment.getUser().getId()) != null)
                return commentTableRepository.update(comment.getStar(), comment.getComment(), tableId,
                        comment.getUser().getId()) > 0;
            return commentTableRepository.save(comment.getStar(), comment.getComment(), tableId,
                    comment.getUser().getId(), comment.getUser().getUsername()) >= 1;
        } catch (Exception e) {
            e.printStackTrace();
        }
        return false;
